...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operator, MacroProcedure):
            ...
            rest = rest.map(lambda a: scheme_eval(a, env))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        name, params = target.first, expressions.first.second
        new_expr = Pair(params, expressions.second)
        value = do_lambda_form(new_expr, env) 
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if not clause.second:
                return test
            return do_and_form(clause.second, env)
            # END PROBLEM 14
        expressions = expressions.second
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    def help_formals(bindings): 
        if not bindings: 
            return nil
        else:
            check_form(bindings.first, 2, 2)
            return Pair(bindings.first.first, help_formals(bindings.second))

    def help_vals(bindings):
        if not bindings: 
            return nil
        else: 
            check_form(bindings.first, 2, 2)
            current = bindings.first.second.first
            return Pair(scheme_eval(current, env), help_vals(bindings.second))

    formals = help_formals(bindings)
    check_formals(formals)
    vals = help_vals(bindings)
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 21
    check_form(expressions, 2)
    if scheme_symbolp(expressions.first.first) and isinstance(expressions.first, Pair):
        formals = expressions.first.second
        body = expressions.second
        new_procedure = MacroProcedure(formals, body, env)
        env.define(expressions.first.first, new_procedure)
