...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operator, MacroProcedure):
            ...
            list_evaluated = rest.map(lambda x: scheme_eval(x, env))
            return scheme_apply(operator, list_evaluated, env)
        # END PROBLEM 5
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('too many/too few vals')
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("Scheme Error: you suck big time dude")
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        lamb_expression = Pair(target.second, expressions.second)
        env.define(target.first, do_lambda_form(lamb_expression, env))
        return target.first
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions.second != nil:
        current_eval = scheme_eval(expressions.first, env)
        if scheme_falsep(current_eval):
            return current_eval
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    let_names, let_expressions = nil, nil
    while bindings is not nil:
        check_form(bindings.first, 2, 2)
        let_names = Pair(bindings.first.first, let_names)
        check_formals(let_names)
        let_expressions = Pair(scheme_eval(bindings.first.second.first, env), let_expressions)
        bindings = bindings.second
    return Frame.make_child_frame(env, let_names, let_expressions)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        rest_expr = target.second
        func = expressions.second
        macro = MacroProcedure(rest_expr, func, env)
...
class Promise(object):
    ...
    def evaluate(self):
        if self.expression is not None:
            value = scheme_eval(self.expression, self.env.make_child_frame(nil, nil), True)
