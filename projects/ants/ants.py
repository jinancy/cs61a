...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        n, k = self.place, 0
        while n != hive:
            if n.bees and k <= self.max_range and k >= self.min_range:
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        if bee and self.digesting == self.time_to_digest - self.time_to_digest:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing each turn. Useful because it has a large armor value."""
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.true_queen:
            ...
            k = self.place.exit
            while k:
                if k.ant and k.ant.is_container:
                    if k.ant not in self.ants_doubled:
                        k.ant.damage = (k.ant.damage * 2)
                        self.ants_doubled.append(k.ant)
                    if k.ant.contained_ant and k.ant.contained_ant not in self.ants_doubled:
                        k.ant.contained_ant.damage = (k.ant.contained_ant.damage * 2)
                        self.ants_doubled.append(k.ant.contained_ant)
                else:
                    if k.ant and k.ant not in self.ants_doubled:
                        k.ant.damage = (k.ant.damage * 2)
                        self.ants_doubled.append(k.ant)
                k = k.exit
        else:
...
def make_scare(action, bee):
    ...
    def action_scare(colony):
        if not bee.previous_scare:
            bee.direction = False
            bee.previous_scare = True
        action(colony)
    return action_scare
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    action_orig = bee.action
    action_eff = effect(bee.action, bee)
    def action_new(colony):
        nonlocal duration
        if duration > 0:
            action_eff(colony)
            duration -= 1
