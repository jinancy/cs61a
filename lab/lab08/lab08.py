...
def deep_len(lnk):
    ...
    #return sum([deep_len(r) for r in lnk.rest])
...
def make_to_string(front, mid, back, empty_repr):
    ...
    def convert_lst(lnk):
        if lnk is Link.empty:
            return empty_repr
        return front + str(lnk.first) + mid + convert_lst(lnk.rest) + back
    return convert_lst
...
def long_paths(tree, n):
    ...
    if n <= 0 and tree.is_leaf():
        lst_paths.append(Link(tree.label))
    for b in tree.branches:
        for p in long_paths(b, n - 1):
            lst_paths.append(Link(tree.label, p))
    return lst_paths
...
def make_advanced_counter_maker():
    ...
    def keep_count():
        count_total = 0
        def which_call(call):
            nonlocal count_total, global_count_total
            if call == "count":
                count_total += 1
                return count_total
            if call == "global-count":
                global_count_total += 1
                return global_count_total
            if call == "reset":
                count_total = 0
            if call == "global-reset":
                global_count_total = 0
        return which_call
    return keep_count
